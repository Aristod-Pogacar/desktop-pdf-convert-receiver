const { contextBridge, ipcRenderer } = require("electron");

contextBridge.exposeInMainWorld("electron", {
    onIP: (cb) => ipcRenderer.on("ip-address", (_, ip) => cb(ip)),
    onUpdate: (cb) => ipcRenderer.on("update-folder", (_, data) => cb(data)),
    onTransferTimeout: (cb) => ipcRenderer.on("transfer-timeout", (_, data) => cb(data))
});
contextBridge.exposeInMainWorld("api", {
    setPassword: (password) => ipcRenderer.send("set-password", password),
    openExcel: () => ipcRenderer.invoke("open-excel"),
    openPdfFolder: () => ipcRenderer.invoke("open-pdf-folder"),
    resumeTransfer: (matricule, currentCount) => ipcRenderer.invoke("resume-transfer", matricule, currentCount),
    clearTransfer: (matricule) => ipcRenderer.invoke("clear-transfer", matricule)
});
